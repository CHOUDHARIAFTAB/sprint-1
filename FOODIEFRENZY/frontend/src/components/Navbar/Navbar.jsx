...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    // Also update auth state on location change
  ...
  const handleLoginSuccess = (token) => {
    ...
    // 2. Update the component's state to reflect the login
    setIsAuthenticated(true);

    // 3. Navigate the user to the homepage
    navigate('/');
  };

  const handleLogout = () => {
    localStorage.removeItem('authToken');
    // Also remove the old loginData key for cleanliness
    localStorage.removeItem('loginData');
    setIsAuthenticated(false);
    // Optional: navigate to home on logout
    navigate('/');
  };
